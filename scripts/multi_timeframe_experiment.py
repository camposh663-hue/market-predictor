"""Test whether multi-timeframe context (4h/1d RSI + trend) improves the
15m->30min XGBoost winner over the single-timeframe baseline (51.6% dev /
51.8% holdout directional accuracy -- see docs/ESTADO_DEL_PROYECTO.md
section 4.10), and if so, re-run confidence threshold selection and the
cost-aware backtest with it.

Follows the same dev/holdout discipline as scripts/threshold_search.py: the
holdout is only scored once, and only if the dev-set walk-forward result
with the expanded feature set actually clears the single-timeframe
baseline. If it doesn't, touching the holdout would spend the one honest
look this project's methodology allows on a configuration that already
lost on dev -- so the script stops before phase 2.

Run with: python -m scripts.multi_timeframe_experiment
"""
...
CONTEXT_TIMEFRAMES = (TimeFrame.FOUR_HOURS, TimeFrame.ONE_DAY)
N_SPLITS = 5
TEST_SIZE = 0.15
...
# multi-timeframe context must clear before the holdout is touched.
...
def main() -> None:
    ...
    context_bars = {tf: _load_bars(data_manager, tf) for tf in CONTEXT_TIMEFRAMES}
    print(f"Loaded {len(bars_15m)} {TIMEFRAME.value} bars.")
    for tf, bars in context_bars.items():
        print(f"Loaded {len(bars)} {tf.value} context bars.")

    feature_calculator = MultiTimeframeFeatureCalculator(
    ...
    print("\nPhase 1 -- walk-forward dev accuracy with multi-timeframe features:")
    ...
    if mean_da <= BASELINE_DEV_ACCURACY:
        print(
            "\nMulti-timeframe context did not beat the single-timeframe dev "
    ...
    model_path = MODELS_DIR / f"best_{final_model.model_id}_15m_03000_multi_timeframe.joblib"
