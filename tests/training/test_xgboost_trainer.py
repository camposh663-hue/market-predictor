...
class XGBoostTrainerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.trainer = XGBoostTrainer(n_estimators=10, max_depth=2)

    def test_model_id(self) -> None:
        self.assertEqual(self.trainer.model_id, "xgboost")
    ...
    def test_feature_importances_are_sorted_descending(self) -> None:
        X, y = _dataset()
        self.trainer.fit(X, y)
        ...
        self.assertEqual(
            ...
            sorted(importances.index, key=lambda name: -importances[name]),
        )
